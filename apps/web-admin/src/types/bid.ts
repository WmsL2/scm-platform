...
export type BidImportStatus = "PARSED" | "MAPPING_REQUIRED" | "FAILED"
export type BidItemStatus = "PENDING" | "NO_MATCH" | "UNIQUE_MATCH" | "MULTIPLE_MATCH" | "SELECTED" | "NO_QUOTE"
export type BidFileType = "ORIGINAL" | "QUOTED_EXPORT"
export type NoQuoteReason = "NO_PRODUCT_MATCH" | "BRAND_MODEL_MISMATCH" | "PRICE_NOT_MATCH" | "SUPPLIER_UNAVAILABLE" | "OTHER"

export interface BidProjectListItem { id: string; project_code: string; project_name: string; buyer_name: string; start_at: string | null; deadline_at: string | null; status: BidProjectStatus; import_status: BidImportStatus; total_item_count: number; processed_item_count: number; created_at: string }
...
export interface BidProjectCreateResult { id: string; project_code: string; status: BidProjectStatus; import_status: BidImportStatus; import_error: string | null; template_id: string | null; template_version: number | null; total_item_count: number }
...
export interface BidCandidate { candidate_id: string; product_id: string; supplier_id: string; product_name: string | null; brand: string | null; model: string | null; category_path: string; product_specification: string | null; supplier_code: string; supplier_name: string; cost_price: string; jd_price: string | null; agreement_price: string | null; score: string; rank: number; method: string; match_reason: Record<string, unknown> }
...
export const IMPORT_STATUS_LABELS: Record<string, string> = { PARSED: "已解析", MAPPING_REQUIRED: "待模板配置", FAILED: "导入失败" }
...
export const FILE_TYPE_LABELS: Record<string, string> = { ORIGINAL: "原始文件", QUOTED_EXPORT: "报价文件" }
