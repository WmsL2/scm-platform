...
export interface ManualCheck { code: string; label: string; requirement_text: string; required: boolean; status: "PENDING" | "PASS" | "FAIL"; evidence: string | null }
export interface ParsedRequirement { gross_margin_min: string | null; jd_price_min: string | null; jd_price_max: string | null; category_keywords: string[]; brand_keywords: string[]; scenario_keywords: string[]; fulfillment_mode: string | null; explicit_category_keywords?: string[]; category_intents?: string[]; excluded_category_keywords?: string[]; required_brands?: string[]; preferred_brands?: string[]; excluded_brands?: string[]; search_keywords?: string[]; scenarios?: string[]; promotion_preference?: string | null; demand_mode?: string | null; quantity?: number | null; manual_checks?: ManualCheck[] }
...
export interface RecommendationCandidate { id: string; product_id: string; rank: number; score: string | null; reason: string | null; manual_flags?: { checks?: ManualCheck[] } | null; product_snapshot: Record<string, unknown>; supplier_snapshot: Record<string, unknown>; price_snapshot: Record<string, unknown>; factory_direct: FactoryDirectStatus | null; confirmation: RecommendationConfirmation | null }
...
export const TEMPLATE_MAPPING_FIELDS: Array<{ key: string; label: string }> = [
  ...
  { key: "supports_jd_or_sf", label: "是否支持京东或者顺丰物流（派生）" },
