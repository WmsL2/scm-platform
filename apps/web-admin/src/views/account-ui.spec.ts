...
describe("account UI contracts", () => {
  ...
  it("uses independent permission checks and dynamic management dialogs", () => {
    ...
    expect(source(RolesView)).toContain("permissionGroups")
    expect(source(RolesView)).toContain("toggleModule")
    expect(render(RolesView)).toContain("el-collapse")
    expect(render(RolesView)).toContain("isModuleIndeterminate")
