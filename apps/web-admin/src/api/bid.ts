...
import type { BidCandidate, BidFileType, BidItemPage, BidItemStatus, BidProjectCreateResult, BidProjectDetail, BidProjectFile, BidProjectPage, BidProjectStatus, BidProjectStatusResult, BidSelectionResult, NoQuoteReason, StartMatchingResult } from "../types/bid"
...
export const bidApi = {
  ...
  create(input: { file: File; project_name: string; buyer_name: string; start_at?: string; deadline_at?: string; remark?: string }): Promise<BidProjectCreateResult> { const form = new FormData(); form.append("file", input.file); form.append("project_name", input.project_name); form.append("buyer_name", input.buyer_name); if (input.start_at) form.append("start_at", input.start_at); if (input.deadline_at) form.append("deadline_at", input.deadline_at); if (input.remark?.trim()) form.append("remark", input.remark.trim()); return http.post(`${base}`, form, { timeoutMs: 300_000 }) },
