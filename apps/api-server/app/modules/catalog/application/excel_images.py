...
def extract_dispimg_images(file_bytes: bytes) -> dict[str, ExcelImage]:
    ...
    try:
        ...
        return {}
    with archive:
        return _extract_dispimg_images(archive)


def extract_dispimg_images_from_path(source: Path) -> dict[str, ExcelImage]:
    """Extract DISPIMG images without loading the complete workbook into memory."""
    try:
...
def _extract_dispimg_images(archive: zipfile.ZipFile) -> dict[str, ExcelImage]:
    try:
