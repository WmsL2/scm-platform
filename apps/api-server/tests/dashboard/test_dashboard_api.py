...
async def test_dashboard_summary_counts_formal_products_and_archived_suppliers() -> None:
    ...
    try:
        ...
        async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
            ...
            assert after.json()["data"] == {
                "formal_product_count": before_data["formal_product_count"] + 1,
                "archived_supplier_count": before_data["archived_supplier_count"] + 2,
            }
    finally:
        async with SessionLocal() as session:
