...
状态：IMPLEMENTED / PRODUCT_IMPORT_SAFETY_CONCURRENCY
...
- [x] `20260918_0031` 增加导入行标准化正式值及 Product 乐观并发快照
...
`IMPLEMENTED / PRODUCT_MASTER_2026_TEMPLATE_AND_FILTERS`：批准模板已升级为 43 列；新增字段、覆盖式重新导入、旧图片回收、组合筛选、三级类目直接多选与父级回显，以及首次不下载六千余条类目、远程选项每批最多 50 条且下拉续载已实现。空或不合格供应商、无效类目和停用同键商品仍不得绕过导入校验。
