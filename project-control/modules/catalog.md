...
- [x] `20260921_0036` 为按状态查询的京东价、利润区间新增组合索引；不改变商品业务字段或价格计算规则
- [x] `20260921_0037` 为按状态查询的所属公司多选新增组合索引；不改变商品业务字段或价格计算规则
...
- [x] 固定 43 列 2026 商品大表导入（仅 SKU 与供应商必填，其余 41 列含三级类目可为空）、直接保存价格正式值、保存 WPS/Excel 内嵌图片、严格供应商精确匹配、SKU 防重预览和通过行原子 Confirm
...
## Frontend
- [x] 商品列表筛选支持一级、二级、三级类目直接搜索和多选；候选项按层级从可见 Product Master 的非空路径取得，选择三级时自动回显对应一级、二级，选择二级时自动回显一级。所属公司、采销员、品牌、供应商同样支持远程搜索和多选；所有远程候选首次每批最多 50 条，下拉接近底部自动续载下一批，同字段为 OR、与其他筛选为 AND，供应商以正式 ID 筛选。成本价、协议价、京东价、利润、折扣率和销量均支持闭区间。商品图片、SKU、商品名称固定为前三列，其余完整业务列按勾选先后显示并保存在浏览器本地。商品编辑页的三级类目文本可独立留空并提交 `null`；供应商/SKU 只读，图片通过文件上传维护
...
- Product Import 与手工编辑均使用 Product 自有的三级类目文本；三个字段均可为空，不写入、查询或依赖 Category ID；
- 一期不强制 SPU/SKU；
...
- 69码按源文本原样保存，不拆分；
- 类目维表来源规则仍适用于独立类目管理，但不再限制商品大表；商品大表类目可为空，非空路径允许不存在或重复；
...
- 导出继续复用 `product:list`
- 单次最多导出 5000 个商品
- 商品自选字段 Excel 导出请求的前端超时从 60 秒调整为 5 分钟；仅影响浏览器等待，不改变后端同步导出实现或其他导入接口超时
- 商品导出成功后提示用户文件已开始下载；导出表头按所选字段复用正式商品主数据下载模板的对应样式及表头行高
- 商品导出数据区域的非空、空白、日期及图片单元格均写入细边框；格式对象复用，避免逐单元格创建样式
- 商品自选字段导出的七个价格／金额列在文件中四舍五入至两位小数，普通金额写为数值单元格并显示 `0.00`；不改数据库原值、百分比、SKU 或正式导入模板
- 商品导入预览／确认、自选字段导出和不通过行导出显示当前请求耗时，模板下载不计时；导入遮罩同步更新分钟秒数，成功／失败后保留本次耗时
- 商品自选字段 Excel 导出支持用户主动取消：弹窗取消、右上角关闭和 ESC 均中止当前浏览器导出请求；取消后不创建下载链接，Operation Timer 显示已取消
