...
- [x] Supplier Master 新增、改名和 Excel 预览/确认使用相同的主体文字判重键：NFKC + 大小写折叠，仅保留字母数字；中文/英文括号、空格和标点差异不再绕过校验，原始名称仍原样保存；商品来源供应商匹配继续遵循 ADR-0008 的严格规则
...
- 主体文字判重第一版为应用层比较，尚无数据库标准化键唯一约束；极端并发下两个不同标点写法同时创建仍有竞争窗口。历史已存在的主体同名记录不会自动合并；如多个逻辑删除记录命中同一主体键，拒绝自动恢复。
