...
- 带 `%` 的比例按百分数除以 100，例如 `46.25% -> 0.4625`；不带 `%` 的比例按数据库小数值解释。
  比例数值和公式缓存结果先按单元格数字格式的显示精度取值，再统一四舍五入到数据库 4 位小数；
  利润公式结果按单元格数字格式的显示精度取值，以消除 `4.40000000000001` 一类 Excel 浮点尾差；
  成本价、市场价、京东价、协议价、协议价采购价及京东自营前台价保留 Excel 底层原始数值，
  不按单元格显示格式四舍五入，字段精度遵循 ADR-0028；
  空值为 `NULL`；非法文本、利润中的 `%`、没有缓存结果或结果为 `#DIV/0!` 等错误的数值公式均使该行不通过。
...
- 新增 Revision `20260918_0033`，基于类目解耦后的 `20260918_0032`，仅扩展 Staging 行，不修改正式商品、供应商或账号。
- `0033` 会检测安全字段和索引是否已存在，以兼容本功能分支在合并主分支前曾以临时 `0031` 编号升级过的开发库；不会覆盖既有标准化值或快照。
...
- Confirm 的业务键查询与锁定按稳定顺序每 500 组分批执行，避免 MySQL 的超大复合 `IN` 范围优化内存阈值；前端请求超时为 15 分钟，不能因浏览器默认短超时把已提交的 Confirm 误报为失败。
