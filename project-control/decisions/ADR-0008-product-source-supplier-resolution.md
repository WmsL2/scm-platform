...
状态：ACCEPTED（Quote 相关表述由 ADR-0009 替代）
日期：2026-09-09
...
- `source_supplier_id` 表示来源供应商，不表示该商品唯一供应商，也不表示当前报价供应商；Product Import 不创建独立报价记录。
...
正式 Product 获得可审计、稳定的来源供应商关联。独立 Quote 领域已由 ADR-0009 取消；本 ADR 其余来源供应商解析规则继续有效。本 ADR 仅冻结文档和 Schema 决策；未创建 Migration、ORM、API 或 UI。
