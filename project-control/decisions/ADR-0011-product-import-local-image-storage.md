...
> 公式存在但内嵌媒体缺失时的“仅警告”规则已由 ADR-0029 替代；当前规则为该行不通过。
...
开发者克隆项目后可在自己的 `local-data/` 中独立保存和查看 Confirm 后的导入图片，不会把本机媒体写入 Git。图片保存时机、临时源 Excel 和未确认任务的精确键清理规则由 ADR-0017 冻结。
