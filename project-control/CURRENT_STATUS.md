...
- Bid Matching / Task 2：确定性商品匹配基础已在 `feat/bid-matching-selection` 实现，包含受控 Product/Supplier 状态过滤、编码/品牌型号/名称规格类目召回、稳定评分与候选选择/无法报价 DTO。当前 `main` 尚无 1 号任务的投标共享表和 Migration，因此候选持久化、追加式 Selection 快照、真实 API、权限和前端尚未实现；依赖已写入 matching Handoff。
