...
- Category management list now supports server-side AND filters for level-1, level-2, level-3 and business-unit contains matches, plus exact Decimal `deduction_rate` and `is_active` matches. The UI converts its purchase coefficient to `deduction_rate` before querying; count and paged list share the same SQL predicates.
