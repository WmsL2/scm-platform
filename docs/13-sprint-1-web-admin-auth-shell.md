...
已打开的工作区标签基于 SortableJS 支持横向拖拽排序。拖动使用半透明 fallback 浮层而不是浏览器原生 HTML5 拖放，因此鼠标越过标签边界时会继续触发前后换位，不显示系统禁止标志；原列表保留完全不透明的深蓝等宽虚线占位块，激活和非激活标签均强制使用一致的高对比拖动状态。经过其他标签时由排序动画将两侧标签挤开，拖动副本继续跟随鼠标。关闭图标不参与拖动，仍可正常点击。

排序偏好以路径列表保存在当前浏览器；页面刷新后，业务页面在再次打开时会按保存的相对顺序排列。退出登录仍关闭当前业务标签，但保留排序偏好；该偏好不包含业务数据、Token 或页面内容。

浏览器禁止本地存储时，拖拽在当前页面内仍然生效，只是不跨刷新保留。工作台标签仍不可关闭，但可以参与排序。
...
工作台通过正式 Dashboard API 展示商品、正常合作供应商、进行中项目和最近更新的 5 个项目。待处理事项因业务处理入口尚未完整跑通，暂不读取数据并显示 `--`；快捷入口按当前账号权限显示。
