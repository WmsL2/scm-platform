...
- 单独的“更新成本价”操作仍调用 Pricing Service 并原子重算派生字段。
...
| 14 | 市场价 | `market_price` | `DECIMAL(18,4)` | 直接保存 |
...
| 31 | 京东自营前台价 | `jd_self_operated_price` | `DECIMAL(18,4)` | 直接保存 |
...
- 类目使用三个独立且可搜索的下拉框：一级、二级、三级。选择一级后才可选择该一级下的二级；选择二级后才可选择该路径下的三级，最终按 `category_id` 精确过滤。
