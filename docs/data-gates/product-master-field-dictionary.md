...
- 所属公司、采销员、品牌和供应商使用可远程搜索的多选候选项：候选均从当前状态下可见的正式 Product 去重取得，供应商提交受控 `source_supplier_id`，不以名称作为正式外键；同一字段多个选择按 OR，其余筛选条件按 AND。聚焦或搜索时每批最多获取 50 条，下拉接近底部自动以 `offset` 继续获取下一批，直到 `has_more` 为否。
...
- 成本价、协议价、京东价、利润、折扣率和销量使用包含边界的最小值/最大值筛选；利润为金额字段 `profit`，不混用京东价毛利、扣点复核或毛利率。页面折扣率输入 `80` 表示 `0.8`。
- 综合搜索覆盖商品名、品牌、型号、SKU、货号、69码、规格、卖点、三级类目路径、所属公司、采销员和供应商名称；所有筛选条件按 AND 组合。
- 列表的商品图片、SKU、商品名称固定为最左侧前三列，不能取消；其余可选业务列（含利润、京东价毛利、扣点复核、毛利率及完整商品资料）按用户勾选先后显示。设置保存在浏览器本地；详情与编辑展示全部业务字段。
...
错误预览中的未处理不通过行使用当前正式商品主数据下载模板导出，模板样式不重建；第二工作表记录原 Excel 行号与错误原因。运营修正首个工作表后可直接重新上传。导出不会自动修复字段，也不会把错误行写入正式商品表。
